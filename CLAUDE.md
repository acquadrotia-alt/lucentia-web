...
- `AgendaView` has two day layouts, switched by the Elenco/Studio toggle: the card list, and `VistaStudio`, a timeline where **each row is a client** and the blocks are the resources following them. That distinction matters because with phased services a salon can have six clients at once on three operators — some in posa, some alone in a cabin — which a list cannot show. The operator dropdown filters both: "Tutti gli operatori" is the studio view, a name is that person's day (their own blocks full colour, the rest dimmed). Studio is day-only; switching to 3 giorni/Settimana falls back to the list.
